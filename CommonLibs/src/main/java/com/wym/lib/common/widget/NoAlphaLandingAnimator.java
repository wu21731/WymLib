 ...
 * date:    2017/10/11 10:10
 * desc:    去掉闪烁效果的Landing动画，要去掉默认的闪烁效果必须要修改animateChange和animateChangeImpl的方法
 */

public class NoAlphaLandingAnimator extends SimpleItemAnimator {
    ...
    public void runPendingAnimations() {
        ...
        if (movesPending) {
            ...
            Runnable mover = new Runnable() {
                @Override
                public void run() {
                    for (NoAlphaLandingAnimator.MoveInfo moveInfo : moves) {
        ...
        if (changesPending) {
            ...
            Runnable changer = new Runnable() {
                @Override
                public void run() {
                    for (NoAlphaLandingAnimator.ChangeInfo change : changes) {
    ...
    protected void animateRemoveImpl(final RecyclerView.ViewHolder holder) {
        ViewCompat.animate(holder.itemView)
                ...
                .setListener(new NoAlphaLandingAnimator.DefaultRemoveVpaListener(holder))
    ...
    protected void animateAddImpl(final RecyclerView.ViewHolder holder) {
        ViewCompat.animate(holder.itemView)
                ...
                .setInterpolator(mInterpolator)
                .setListener(new NoAlphaLandingAnimator.DefaultAddVpaListener(holder))
    ...
    private void doAnimateRemove(final RecyclerView.ViewHolder holder) {
        if (holder instanceof AnimateViewHolder) {
            ((AnimateViewHolder) holder).animateRemoveImpl(holder, new NoAlphaLandingAnimator.DefaultRemoveVpaListener(holder));
        } else {
    ...
    private void doAnimateAdd(final RecyclerView.ViewHolder holder) {
        if (holder instanceof AnimateViewHolder) {
            ((AnimateViewHolder) holder).animateAddImpl(holder, new NoAlphaLandingAnimator.DefaultAddVpaListener(holder));
        } else {
    ...
    public boolean animateMove(final RecyclerView.ViewHolder holder, int fromX, int fromY, int toX, int toY) {
        ...
        mPendingMoves.add(new NoAlphaLandingAnimator.MoveInfo(holder, fromX, fromY, toX, toY));
        return true;
    }
    ...
    private void animateMoveImpl(final RecyclerView.ViewHolder holder, int fromX, int fromY, int toX, int toY) {
        ...
        animation.setDuration(getMoveDuration()).setListener(new NoAlphaLandingAnimator.VpaListenerAdapter() {
            @Override
...
//            ViewCompat.setAlpha(newHolder.itemView, 0);
        ...
        mPendingChanges.add(new NoAlphaLandingAnimator.ChangeInfo(oldHolder, newHolder, fromX, fromY, toX, toY));
        return true;
    }

    private void animateChangeImpl(final NoAlphaLandingAnimator.ChangeInfo changeInfo) {
        ...
        if (view != null) {
            ...
            oldViewAnim
//                    .alpha(0)
                    .setListener(new NoAlphaLandingAnimator.VpaListenerAdapter() {
                        @Override
                        public void onAnimationStart(View view) {
...
//                    .alpha(1)
    ...
    private void endChangeAnimation(List<ChangeInfo> infoList, RecyclerView.ViewHolder item) {
        for (int i = infoList.size() - 1; i >= 0; i--) {
            NoAlphaLandingAnimator.ChangeInfo changeInfo = infoList.get(i);
    ...
    private void endChangeAnimationIfNecessary(NoAlphaLandingAnimator.ChangeInfo changeInfo) {
    ...
    private boolean endChangeAnimationIfNecessary(NoAlphaLandingAnimator.ChangeInfo changeInfo, RecyclerView.ViewHolder item) {
    ...
    public void endAnimation(RecyclerView.ViewHolder item) {
        ...
        for (int i = mPendingMoves.size() - 1; i >= 0; i--) {
            NoAlphaLandingAnimator.MoveInfo moveInfo = mPendingMoves.get(i);
        ...
        for (int i = mMovesList.size() - 1; i >= 0; i--) {
            ...
            for (int j = moves.size() - 1; j >= 0; j--) {
                NoAlphaLandingAnimator.MoveInfo moveInfo = moves.get(j);
                if (moveInfo.holder == item) {
                    ViewCompat.setTranslationY(view, 0);
                    ViewCompat.setTranslationX(view, 0);
                    dispatchMoveFinished(item);
    ...
    public void endAnimations() {
        ...
        for (int i = count - 1; i >= 0; i--) {
            NoAlphaLandingAnimator.MoveInfo item = mPendingMoves.get(i);
    ...
    protected class DefaultAddVpaListener extends NoAlphaLandingAnimator.VpaListenerAdapter {
    ...
    protected class DefaultRemoveVpaListener extends NoAlphaLandingAnimator.VpaListenerAdapter {
